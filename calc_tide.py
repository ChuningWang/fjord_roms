...
ubar = np.zeros((nt, eta, xi-1))
vbar = np.zeros((nt, eta-1, xi))
...
ubar2 = np.zeros((nt, eta, xi-1))
vbar2 = np.zeros((nt, eta-1, xi))
...
for i in range(fnum):
    ...
    if i == 0:
        ...
        ubar[:snum+1, :, :] = fin.variables['ubar'][:]
        vbar[:snum+1, :, :] = fin.variables['vbar'][:]
    else:
        ...
        ubar[i*snum+1:(i+1)*snum+1, :, :] = fin.variables['ubar'][:]
        vbar[i*snum+1:(i+1)*snum+1, :, :] = fin.variables['vbar'][:]
    fin.close()
    ...
    if i == 0:
        ...
        ubar2[:snum+1, :, :] = fin.variables['ubar'][:]
        vbar2[:snum+1, :, :] = fin.variables['vbar'][:]
    else:
        ...
        ubar2[i*snum+1:(i+1)*snum+1, :, :] = fin.variables['ubar'][:]
        vbar2[i*snum+1:(i+1)*snum+1, :, :] = fin.variables['vbar'][:]
    fin.close()
...
ubar = np.ma.masked_where(msku3d, ubar)
vbar = np.ma.masked_where(mskv3d, vbar)
...
ubar2 = np.ma.masked_where(msku3d, ubar2)
vbar2 = np.ma.masked_where(mskv3d, vbar2)
