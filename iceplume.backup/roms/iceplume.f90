...
! ==================================================================
SUBROUTINE iceplume_calc(ng, wIni, rIni, TIni, SIni)
  ...
  IF (QIni .GT. 0) THEN
    ...
    DO k = 1, Nr
      ...
      IF (K .GT. iceDepthK) THEN
        ...
        IF (useSheetPlume) THEN
          PLUME(ng) % volFlux(K) = (PLUME(ng) % r(K))*(PLUME(ng) % w(K))*dy
        ELSEIF (useConePlume) THEN
          PLUME(ng) % volFlux(K) = pi * ((PLUME(ng) % r(K))**2) * &
            & (PLUME(ng) % w(K)) / 2.
        ELSE
    ...
    DO K = iceDepthK, Nr
      PLUME(ng) % ent(K) = -(PLUME(ng) % volFlux(K+1) - PLUME(ng) % volFlux(K))
    ENDDO
  ...
  DO K = 1, Nr
    ...
    ELSE
      ...
      PLUME(ng) % tendT(K) = -(PLUME(ng) % heatFlux(K))/c_i/rho_ref
      PLUME(ng) % tendS(K) = (PLUME(ng) % fwFlux(K))*(PLUME(ng) % sAm(K))/rho_ref
  ...
  IF (useTracers) THEN
    ...
    IF (useInputTracers) THEN
      DO iTracer = 3, NT(ng)
        PLUME(ng) % trcCum(iTracer) = PLUME(ng) % trcCum(iTracer) + &
...
SUBROUTINE iceplume_plume_model(ng, rIni, wIni, tIni, sIni)
  ...
  DO K = 1, Nr
    ...
    PLUME(ng) % a(K) = 0.0
    PLUME(ng) % mInt(K) = 0.0
    PLUME(ng) % zRhoAbs(K) = abs(0.5d0*(PLUME(ng) % zW(K) + &
      &PLUME(ng) % zW(K+1)))
  ENDDO
...
SUBROUTINE  HALFCONE (NEQ, T, Y, YDOT)
  ...
  IF (abs(T) .LE. PLUME(ngr) % zRhoAbs(1)) THEN
    Tambient = PLUME(ngr) % tAm(1)
    Sambient = PLUME(ngr) % sAm(1)
  ELSEIF (abs(T) .GE. PLUME(ngr) % zRhoAbs(Nr)) THEN
    Tambient = PLUME(ngr) % tAm(Nr)
    Sambient = PLUME(ngr) % sAm(Nr)
  ELSE
    CALL linint(Nr, PLUME(ngr) % zRhoAbs, PLUME(ngr) % tAm, T, Tambient)
    CALL linint(Nr, PLUME(ngr) % zRhoAbs, PLUME(ngr) % sAm, T, Sambient)
  ENDIF
