...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 202,                               &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 228,                              &
     &                 "ROMS/Utility/distribute.F"//":mp_bcastf")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 280,                               &
     &                "ROMS/Utility/distribute.F"//":mp_bcastf")
!
!-----------------------------------------------------------------------
!  Broadcast requested variable.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 310,                              &
     &                 "ROMS/Utility/distribute.F"//":mp_bcastf")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 364,                               &
     &                "ROMS/Utility/distribute.F"//":mp_bcastf")
!
!-----------------------------------------------------------------------
!  Broadcast requested variable.
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 449,                               &
     &                "ROMS/Utility/distribute.F"//":mp_bcastf")
!
!-----------------------------------------------------------------------
!  Broadcast requested variable.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 481,                              &
     &                 "ROMS/Utility/distribute.F"//":mp_bcastf")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 535,                               &
     &                "ROMS/Utility/distribute.F"//":mp_bcastf")
!
!-----------------------------------------------------------------------
!  Broadcast requested variable.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 568,                              &
     &                 "ROMS/Utility/distribute.F"//":mp_bcastf")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 619,                               &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 645,                              &
     &                 "ROMS/Utility/distribute.F"//":mp_bcasti")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 697,                               &
     &                "ROMS/Utility/distribute.F"//":mp_bcasti")
!
!-----------------------------------------------------------------------
!  Broadcast requested variable.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 726,                              &
     &                 "ROMS/Utility/distribute.F"//":mp_bcasti")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 779,                               &
     &                "ROMS/Utility/distribute.F"//":mp_bcasti")
!
!-----------------------------------------------------------------------
!  Broadcast requested variable.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 810,                              &
     &                "ROMS/Utility/distribute.F"//":mp_bcasti")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 862,                               &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 889,                              &
     &                 "ROMS/Utility/distribute.F"//":mp_bcastl")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 941,                               &
     &                "ROMS/Utility/distribute.F"//":mp_bcastl")
!
!-----------------------------------------------------------------------
!  Broadcast requested variable.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 970,                              &
     &                 "ROMS/Utility/distribute.F"//":mp_bcastl")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1023,                              &
     &                "ROMS/Utility/distribute.F"//":mp_bcastl")
!
!-----------------------------------------------------------------------
!  Broadcast requested variable.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1054,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_bcastl")
      RETURN
...
!
      IF (Lwclock) THEN
        CALL wclock_on (ng, model, 64, 1106,                            &
...
!
      IF (Lwclock) THEN
        CALL wclock_off (ng, model, 64, 1136,                           &
     &                   "ROMS/Utility/distribute.F"//":mp_bcasts")
      END IF
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1189,                              &
     &                "ROMS/Utility/distribute.F"//":mp_bcasts")
!
!-----------------------------------------------------------------------
!  Broadcast requested variable.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1219,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_bcasts")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 68, 1296,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 68, 1394,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_boundary")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 1463,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 1606,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_assemble")
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 1688,                              &
     &                "ROMS/Utility/distribute.F"//":mp_assemble")
!
!-----------------------------------------------------------------------
!  Check input parameters.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 1846,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_assemble")
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 1928,                              &
     &                "ROMS/Utility/distribute.F"//":mp_assemble")
!
!-----------------------------------------------------------------------
!  Check input parameters.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 2087,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_assemble")
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 2165,                              &
     &                "ROMS/Utility/distribute.F"//":mp_assemble")
!
!-----------------------------------------------------------------------
!  Check input parameters.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 2308,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_assemble")
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 2389,                              &
     &                "ROMS/Utility/distribute.F"//":mp_assemble")
!
!-----------------------------------------------------------------------
!  Check input parameters.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 2548,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_assemble")
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 69, 2625,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 69, 2751,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_collect")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 69, 2820,                              &
     &                "ROMS/Utility/distribute.F"//":mp_collect")
!
!-----------------------------------------------------------------------
!  Collect data from all nodes.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 69, 2946,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_collect")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 3010, "ROMS/Utility/distribute.F")
!
!-----------------------------------------------------------------------
!  Collect data from all nodes.
!-----------------------------------------------------------------------
...
!
        DO rank=1,NtileI(ng)*NtileJ(ng)-1
          CALL mpi_irecv (Arecv, Npts, MPI_LOGICAL, rank, rank+5,       &
     &                    OCN_COMM_WORLD, request, MyError)
          CALL mpi_wait (request, status, MyError)
          IF (MyError.ne.MPI_SUCCESS) THEN
            CALL mpi_error_string (MyError, string, Lstr, Serror)
...
!
      ELSE
        CALL mpi_isend (A, Npts, MPI_LOGICAL, MyMaster, MyRank+5,       &
     &                  OCN_COMM_WORLD, request, MyError)
        CALL mpi_wait (request, status, MyError)
        IF (MyError.ne.MPI_SUCCESS) THEN
          CALL mpi_error_string (MyError, string, Lstr, Serror)
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 3122, "ROMS/Utility/distribute.F")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 3223,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 3450,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_gather2d")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 3554,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 3802,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_gather3d")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 3867,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 3950,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_gather_state")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 4031,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 4156,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_ncread1d")
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 4246,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 4381,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_ncread2d")
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 4469,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 4600,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_ncwrite1d")
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 4690,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 4831,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_ncwrite2d")
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 65, 4906,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 65, 5023,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_reduce")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 65, 5091,                              &
     &                "ROMS/Utility/distribute.F"//":mp_reduce")
!
!-----------------------------------------------------------------------
!  Collect and reduce requested scalar variables.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 65, 5220,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_reduce")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 65, 5285,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 65, 5337,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_reduce2")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 5427,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 5606,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_scatter2d")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 5698,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 5890,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_scatter3d")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 5960,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 6054,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_scatter_state")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 71, 6253,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 71, 6378,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_aggregate2d")
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 71, 6460,                              &
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 71, 6591,                             &
     &                 "ROMS/Utility/distribute.F"//":mp_aggregate3d")
      RETURN
