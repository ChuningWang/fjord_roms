...
!
      CALL wclock_on (ng, IDmod, 42, 185, "ROMS/Utility/get_state.F")
!
...
!
     ...
     &                        OCEAN(ng) % ubar(:,:,Tindex))
          END IF
          IF (FoundError(status, nf90_noerr, 742,                       &
     &                   "ROMS/Utility/get_state.F")) THEN
            IF (Master) THEN
...
!
     ...
     &                      GRID(ng) % umask,                           &
     &                      OCEAN(ng) % rubar)
          IF (FoundError(status, nf90_noerr, 773,                       &
     &                   "ROMS/Utility/get_state.F")) THEN
            IF (Master) THEN
...
!
     ...
     &                      GRID(ng) % vmask,                           &
     &                      OCEAN(ng) % rvbar)
          IF (FoundError(status, nf90_noerr, 851,                       &
     &                   "ROMS/Utility/get_state.F")) THEN
            IF (Master) THEN
...
!
     ...
     &                        OCEAN(ng) % u(:,:,:,Tindex))
          END IF
          IF (FoundError(status, nf90_noerr, 896,                       &
     &                   "ROMS/Utility/get_state.F")) THEN
            IF (Master) THEN
...
!
     ...
     &                          OCEAN(ng) % t(:,:,:,Tindex,itrc))
            END IF
            IF (FoundError(status, nf90_noerr, 1045,                    &
     &                     "ROMS/Utility/get_state.F")) THEN
              IF (Master) THEN
...
!
     ...
     &                      GRID(ng) % rmask,                           &
     &                      MIXING(ng) % AKt(:,:,:,itemp))
          IF (FoundError(status, nf90_noerr, 1273,                      &
     &                   "ROMS/Utility/get_state.F")) THEN
            IF (Master) THEN
...
!
     ...
     &                      GRID(ng) % rmask,                           &
     &                      MIXING(ng) % AKt(:,:,:,isalt))
          IF (FoundError(status, nf90_noerr, 1312,                      &
     &                   "ROMS/Utility/get_state.F")) THEN
            IF (Master) THEN
...
!
     ...
     &                      GRID(ng) % rmask,                           &
     &                      MIXING(ng) % tke)
          IF (FoundError(status, nf90_noerr, 1461,                      &
     &                   "ROMS/Utility/get_state.F")) THEN
            IF (Master) THEN
...
!
     ...
     &                      GRID(ng) % rmask,                           &
     &                      MIXING(ng) % gls)
          IF (FoundError(status, nf90_noerr, 1492,                      &
     &                   "ROMS/Utility/get_state.F")) THEN
            IF (Master) THEN
...
!
     ...
     &                      GRID(ng) % rmask,                           &
     &                      MIXING(ng) % Lscale)
          IF (FoundError(status, nf90_noerr, 1523,                      &
     &                   "ROMS/Utility/get_state.F")) THEN
            IF (Master) THEN
...
!
     ...
     &                      GRID(ng) % rmask,                           &
     &                      MIXING(ng) % Akk)
          IF (FoundError(status, nf90_noerr, 1554,                      &
     &                   "ROMS/Utility/get_state.F")) THEN
            IF (Master) THEN
...
!
      CALL wclock_off (ng, IDmod, 42, 6281, "ROMS/Utility/get_state.F")
!
