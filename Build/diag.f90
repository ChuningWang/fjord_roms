...
!
     ...
     &              dt(ng)/Hz(i,j,k)
              ...
              my_mintemp=MIN(my_mintemp,t(i,j,k,nstp,itemp))
            END DO
          END DO
        END DO
!
...
!$OMP CRITICAL (NL_DIAGNOSTICS)
        ...
        ELSE
          ...
          mintemp(ng)=MIN(mintemp(ng),my_mintemp)
...
!
          ...
          END IF
          IF (mintemp(ng) .lt. min_temp) THEN
            exit_flag=1
            IF (Master) print *, 'DIAG temperature trouble ', mintemp(ng), ng
            ii_max = Istr
            jj_max = Jstr
            kk_max = 1
            uv_max = t(Istr,Jstr,1,nstp,itemp)
            DO j=Jstr,Jend
              DO k=1,N(ng)
                DO i=Istr,Iend
                  IF (t(i,j,k,itemp,nstp) > uv_max) THEN
                    ii_max = i
                    jj_max = j
                    kk_max = k
                    uv_max = t(i,j,k,nstp,itemp)
                  END IF
                END DO
              END DO
            END DO
            IF (uv_max.gt.max_speed) THEN
              print *, 'DIAG temp ijk ', uv_max, ii_max, jj_max, kk_max, ng
            END IF
          END IF
        END IF
