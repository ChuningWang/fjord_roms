...
!
      DO ng=1,Ngrids
        DO thread=MyRank,MyRank
          CALL wclock_on (ng, iNLM, 2, 188, "ROMS/Nonlinear/initial.F")
        END DO
      END DO
!$OMP BARRIER
!
...
!$OMP END MASTER
            CALL mp_bcasti (ng, iNLM, exit_flag)
!$OMP BARRIER
            IF (FoundError(exit_flag, NoError, 407,                     &
     &                     "ROMS/Nonlinear/initial.F")) RETURN
          END IF
        END DO
      END IF
...
!$OMP END MASTER
        CALL mp_bcasti (ng, iNLM, exit_flag)
!$OMP BARRIER
        IF (FoundError(exit_flag, NoError, 810,                         &
     &                 "ROMS/Nonlinear/initial.F")) RETURN
      END DO
!
...
!
      DO ng=1,Ngrids
        DO thread=MyRank,MyRank
          CALL wclock_off (ng, iNLM, 2, 1032, "ROMS/Nonlinear/initial.F")
        END DO
!$OMP BARRIER
      END DO
      RETURN
