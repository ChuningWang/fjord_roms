...
!
     ...
     &                     nvd3, t2dgrd, Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 1248,                    &
     &                     "ROMS/Utility/def_quick.F")) RETURN
          END IF
        END DO
!
...
!
     ...
     &                   NF_FOUT, nvd3, u2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1442,                      &
     &                   "ROMS/Utility/def_quick.F")) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd3, v2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1499,                      &
     &                   "ROMS/Utility/def_quick.F")) RETURN
        END IF
!
!-----------------------------------------------------------------------
...
!
        ...
        IF (FoundError(exit_flag, NoError, 2805,                        &
     &                 "ROMS/Utility/def_quick.F")) RETURN
!
!-----------------------------------------------------------------------
...
!
        ...
        IF (FoundError(exit_flag, NoError, 2828,                        &
     &                 "ROMS/Utility/def_quick.F")) RETURN
!
!  Inquire about the variables.
!
        CALL netcdf_inq_var (ng, iNLM, ncname)
        IF (FoundError(exit_flag, NoError, 2834,                        &
     &                 "ROMS/Utility/def_quick.F")) RETURN
!
!  Open quicksave file for read/write.
!
        CALL netcdf_open (ng, iNLM, ncname, 1, QCK(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 2840,                        &
     &                 "ROMS/Utility/def_quick.F")) THEN
